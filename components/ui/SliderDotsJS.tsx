...
const setup = ({ rootId, scroll, interval, infinite }: Props) => {
  ...
  const dotsLine = root?.querySelectorAll(`[${ATTRIBUTES["data-dot-line"]}]`);
  const sliderDots = root?.querySelector("[data-slider-dots='true']");

  if (
    !root || !slider || !items || items.length === 0 || !sliderDots || !dots ||
    dots.length === 0
  ) {
    console.warn(
      "Missing necessary slider attributes. It will not work as intended. Necessary elements:",
      { root, slider, items, rootId, sliderDots, dots },
    );
  ...
  const goToItemY = (index: number, action: boolean) => {
    ...
    console.log("data", item.offsetTop, action);
  };
  ...
  const observer = new IntersectionObserver(
    (elements) =>
      elements.forEach((item) => {
        ...
        const dotLine = dotsLine?.item(index);

        if (item.isIntersecting) {
          dot?.setAttribute("disabled", "");
          dotLine?.setAttribute("disabled", "");
        } else {
          dot?.removeAttribute("disabled");
          dotLine?.removeAttribute("disabled");
        }
  ...
  for (let it = 0; it < (dotsLine?.length ?? 0); it++) {
    dotsLine?.item(it).addEventListener("click", () => goToItem(it));
  }
  ...
  return () => {
    for (let it = 0; it < (dotsLine?.length ?? 0); it++) {
      dotsLine?.item(it).removeEventListener("click", () => goToItem(it));
    }
    for (let it = 0; it < (dots?.length ?? 0); it++) {
